...
class Users:
        ...
        try:
            ...
            with psycopg.connect(**config) as conn:
                with conn.cursor() as cur:
                    ...
                    cur.execute(query, (Users.SCHEMA, table_name))
        ...
        try:
            ...
            with psycopg.connect(**config) as conn:
                with conn.cursor() as cur:
                    ...
                    non_nullable_columns = Users.get_non_nullable_columns()

                    if not non_nullable_columns:
                        print(f"No non-nullable columns found for table {Users.SCHEMA}.{Users.TABLE}.")
                        return []
                    ...
                    """)
                    cur.execute(column_type_query, (Users.SCHEMA, Users.TABLE, category))
                    column_type = cur.fetchone()

                    if not column_type:
                        raise ValueError(f"Column '{category}' does not exist in table {Users.SCHEMA}.{Users.TABLE}.")
    ...
    @staticmethod
    def update(user_id: int, **user_update) -> bool:
        try:
            config = load_config()
            with psycopg.connect(**config) as conn:
                with conn.cursor() as cur:
                    ...
                    values = [user_update[col] for col in columns]
