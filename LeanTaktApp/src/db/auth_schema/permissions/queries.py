...
class Permissions:
        ...
        try:
            ...
            with psycopg.connect(**config) as conn:
                with conn.cursor() as cur:
                    ...
                    cur.execute(query, (Permissions.SCHEMA, table_name))
        ...
        try:
            ...
            with psycopg.connect(**config) as conn:
                with conn.cursor() as cur:
                    ...
                    cur.execute(query, (Permissions.SCHEMA, Permissions.TABLE))
    ...
    @staticmethod
    def create(**permission:dict) -> int | None:
        try:
            config = load_config()
            with psycopg.connect(**config) as conn:
                with conn.cursor() as cur:
                    columns = permission.keys()
                    values = [permission[column] for column in columns]
                    ...
                    query = sql.SQL("""
                        ...
                        RETURNING id;
                    """).format(
                        sql.Identifier(Permissions.SCHEMA),
                        sql.Identifier(Permissions.TABLE),
    ...
    @staticmethod
    def read(permission_id:int) -> dict:
        try:
            config = load_config()
            with psycopg.connect(**config) as conn:
                with conn.cursor() as cur:
                    ...
                    if row:
                        ...
                        permission = dict(zip(column_names, row))
                        return permission
                    else:
                        return {}
        except (Exception, psycopg.DatabaseError) as e:
    ...
    def read_all() -> list|None:
        try:
            ...
            print(f"Error fetching data from {Permissions.SCHEMA}.{Permissions.TABLE}: {e}")
            return None
        ...
        try:
            ...
            with psycopg.connect(**config) as conn:
                with conn.cursor() as cur:
                    ...
                    non_nullable_columns = Permissions.get_non_nullable_columns()

                    if not non_nullable_columns:
                        print(f"No non-nullable columns found for table {Permissions.SCHEMA}.{Permissions.TABLE}.")
                        return []
                    ...
                    """)
                    cur.execute(column_type_query, (Permissions.SCHEMA, Permissions.TABLE, category))
                    column_type = cur.fetchone()

                    if not column_type:
                        raise ValueError(f"Column '{category}' does not exist in table {Permissions.SCHEMA}.{Permissions.TABLE}.")
    ...
    def search_by(value:str, category:str) -> list:
        try:
            ...
            print(f"Error searching for permission: {e}")
            return []    

    @staticmethod
    def update(permission_id: int, **permission_update) -> bool:
        try:
            config = load_config()
            with psycopg.connect(**config) as conn:
                with conn.cursor() as cur:
                    # Prepare the SET clause
                    columns = permission_update.keys()
                    set_clause = sql.SQL(', ').join(
                        sql.SQL("{} = %s").format(sql.Identifier(col)) for col in columns
                    )

                    # Prepare the values for the SET clause and the WHERE clause
                    values = [permission_update[col] for col in columns]
                    values.append(permission_id)  # Add permission_id for the WHERE clause
                    ...
                    updated_permission = cur.fetchone()
                    conn.commit()

                    if updated_permission:
                        print(f"Permission with ID {permission_id} updated successfully.")
                        print("Updated permission:", updated_permission)
                        return True
                    else:
    ...
    @staticmethod
    def delete(permission_id:int) -> dict:
        try:
            config = load_config()
            with psycopg.connect(**config) as conn:
                with conn.cursor() as cur:
                    permission = Permissions.read(permission_id)
                    if not permission:
                    ...
                    print(f"Permission with ID {permission_id} deleted successfully.")
                    return permission  
        except (Exception, psycopg.DatabaseError) as e:
