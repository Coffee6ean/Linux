        ...
        try:
            ...
            with psycopg.connect(**config) as conn:
                with conn.cursor() as cur:
                    ...
                    non_nullable_columns = Versions.get_non_nullable_columns()

                    if not non_nullable_columns:
                        print(f"No non-nullable columns found for table {Versions.SCHEMA}.{Versions.TABLE}.")
                        return []
                    ...
                    """)
                    cur.execute(column_type_query, (Versions.SCHEMA, Versions.TABLE, category))
                    column_type = cur.fetchone()

                    if not column_type:
                        raise ValueError(f"Column '{category}' does not exist in table {Versions.SCHEMA}.{Versions.TABLE}.")
        ...
        try:
            ...
            with psycopg.connect(**config) as conn:
                with conn.cursor() as cur:
                    ...
                    values.append(version_id)  # Add version_id for the WHERE clause
                    ...
                    if updated_version:
                        ...
                        print("Updated version:", updated_version)
                        return True
                    else:
