...
import sys
sys.path.append("../")
from FileAnalyst.config.paths import TEST_PDF_DIR
...
class DataIngestion:
    ...
    def main(auto=True, img_proc=False, input_file_path=None, input_file_basename=None, 
             input_file_extension=None, output_dict={}):
        if auto:
            project = DataIngestion.auto_generate_ins(input_file_path, input_file_basename, input_file_extension)
        else:
        ...
        if project:
            ...
            output_folder = f"{TEST_PDF_DIR}/{DataIngestion.return_valid_date()}"
            ...
            if img_proc:
                ...
                output_dict_filled = project.pdf_to_text(file, output_dict)

            DataIngestion.write_dict_to_json(output_dict_filled, "output_filled", TEST_PDF_DIR)
            return output_dict_filled
    ...
    @staticmethod
    def auto_generate_ins(input_file_path=None, input_file_basename=None, input_file_extension=None):
        ins = DataIngestion(
            input_file_path, 
            input_file_basename, 
    ...
    def _handle_file(input_file_dir:str) -> dict|int:
        ...
        if (input_file_extension in DataIngestion.allowed_extensions):
            return dict(
                path = os.path.dirname(input_file_dir), 
    ...
    def extract_data_from_image(image_path:str, output_path:str, output_basename:str, file_extension:str,
        ...
        try:
            if os.stat(image_path).st_size >= 5000:
                ...
                cleaned_text = DataIngestion._normalize_file_text(text)
    ...
    def jsonify_project_data(text_body:str) -> list:
        ...
        proc_dict = {
            "header": {"page": DataIngestion.project_pages},
            "body": {
        ...
        for i, line in enumerate(text_lines, 1):
            try:
                match = DataIngestion.test_pattern(line)
                if not match:
                    match = DataIngestion.special_pattern(line)
                    if not match:
    ...
    def _repackage_images(self, package_name:str) -> None:
        ...
        for i, image in enumerate(self.sorted_image_list):
            output_path = DataIngestion.repackage_file(image, f"{package_name}_{i+1}")
    ...
    def _slice_images(self, new_directory:str, image_extension:str) -> None:
        for image in self.sorted_image_list:
            ...
            DataIngestion.slice_based_on_color_change(image, output_dir, image_extension)
    ...
    def _write_txt_file(self, output_path:str, target_directory:str="", file_extension:str="txt", mode:str='a') -> None:
        for package in self.sorted_image_list:
            ...
            for image in sliced_images:
                ...
                normalized_text = DataIngestion.extract_data_from_image(
                    image_dir, 
                    output_path, 
                    "project_text", 
                    mode
    ...
    def _write_json_file(self, output_path:str, target_directory:str="", file_extension:str="json", mode:str='a') -> None:
        ...
        for package in self.sorted_image_list:
            ...
            for image in sliced_images:
                ...
                project_dict.append(DataIngestion.jsonify_project_data(normalized_text))
    ...
    def pdf_to_text(self, input_file:str, output_dict:dict) -> dict:
        reader = PdfReader(input_file)
        pages = len(reader.pages)

        for page in range(pages):
            current_page_read = reader.pages[page]
            content = current_page_read.extract_text()
            output_dict["pages"].setdefault(page, {
                "content": content,
                "keywords": {},
                "summary": {}
            })
