...
class PdfImageProcessing:
    ...
    @staticmethod
    def main():
        project = PdfImageProcessing.generate_ins()
    ...
    def generate_ins():
        ...
        return PdfImageProcessing(input_images_path, output_images_path)
    ...
    def repackage_files_in_dir(self, input_dir):
        sorted_files = PdfImageProcessing.bubble_sort_str_list(input_dir)

        for i, file in enumerate(sorted_files):
            file_path = os.path.join(input_dir, file)
            PdfImageProcessing.package_file(file_path, f"package_page_{i}")
    ...
    def _crop_single_image(self, img_dir, new_directory, roi_msg):
        sorted_list = PdfImageProcessing.bubble_sort_str_list(img_dir)
        image = sorted_list[0]
    ...
    def _crop_multiple_images(self, img_dir, new_directory, roi_msg):
        sorted_images = PdfImageProcessing.bubble_sort_str_list(img_dir)
    ...
    def detect_horizontal_lines(self, input_dir, min_line_width=100):
        sorted_packages = PdfImageProcessing.bubble_sort_str_list(input_dir)
        images = []

        for package in sorted_packages:
            package_dir = os.path.join(input_dir, package)
            images.append(PdfImageProcessing.get_type_files(package_dir, "jpg")[0])
    ...
    def color_slice_tasks(self, input_dir, new_directory):
        ...
        for package in sorted_packages:
            package_dir = os.path.join(input_dir, package)
            jpg_files = PdfImageProcessing.get_type_files(package_dir, "jpg")
            if jpg_files:
    ...
    def sharpen_project_images(self, input_dir, directory):
        ...
        for package in sorted_packages:
            ...
            project_images = PdfImageProcessing.bubble_sort_str_list(images_dir)

            for image in project_images:
                image_dir = os.path.join(images_dir, image)
                PdfImageProcessing.sharpen_image(image_dir)
    ...
    def write_wbs_file(self, input_dir, directory, new_directory, mode='a'):
        ...
        for package in sorted_packages:
            ...
            for image in project_images:
                ...
                PdfImageProcessing.extract_text_from_image(image_dir, self.output_path, "test", mode)
    ...
    def clean_project_text(self, input_dir, output_path, output_basename, mode='w'):
        ...
        file_dir = PdfImageProcessing.get_type_files(input_dir, 'txt')[0]
