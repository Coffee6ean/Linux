...
class DataGenerate:
    ...
    def main(auto=True, input_file_path=None, input_file_basename=None, 
        ...
        if auto:
            project = DataGenerate.auto_generate_ins(
                input_file_path, 
                input_file_basename, 
            ...
            project = DataGenerate.generate_ins()
        ...
        module_data = {
            ...
            "logs": {
                "start": DataGenerate.return_valid_date(),
                "finish": None,
                "run-time": None,
                "status": [],
            },
        ...
        if project:
            ref_dict = DataGenerate.read_data_from_json(CLAYCO, "ticket")
        ...
        module_data["logs"]["finish"] = DataGenerate.return_valid_date()
        module_data["logs"]["run-time"] = DataGenerate.calculate_time_duration(
    ...
    @staticmethod
    def generate_ins():
        DataGenerate.ynq_user_interaction(
    ...
    def return_valid_file(input_file_dir:str) -> dict|int:
        ...
        if os.path.isdir(input_file_dir):
            file_dict = DataGenerate._handle_dir(input_file_dir)
        elif os.path.isfile(input_file_dir):
            file_dict = DataGenerate._handle_file(input_file_dir)
    ...
    def _handle_dir(input_file_dir:str, mode:str="r") -> dict|int:
        if mode in ['u', 'r', 'd']:
            ...
            selection = DataGenerate._display_options(dir_list)
    ...
    def _handle_file(input_file_dir:str) -> dict|int:
        ...
        if (input_file_extension in DataGenerate.allowed_extensions):
            return dict(
                path = os.path.dirname(input_file_dir), 
    ...
    def user_option_to_select_from_library(prompt_message:str, category:str):
        lib_data = DataGenerate.read_data_from_library(LIBRARY_METADATA, category)

        if lib_data:
            data_arr = [f"{value.name} {value.middle_name}" for _, value in lib_data.items()]
            selection = DataGenerate._display_options(data_arr)
