...
import sys
sys.path.append("../")
from CpmProcessor.keys.secrets import LIBRARY_METADATA

class DataReference:
    allowed_extensions = ["json", "xlsx", "sql"]
    ...
    def main(auto=True, input_file_path=None, input_file_basename=None, 
        ...
        if auto:
            project = DataReference.auto_generate_ins(
                input_file_path, 
                input_file_basename, 
            ...
            project = DataReference.generate_ins()
        ...
        module_data = {
            ...
            "logs": {
                "start": DataReference.return_valid_date(),
                "finish": None,
                "run-time": None,
                "status": [],
            },
        ...
        if project:
            ref_dict = DataReference.read_data_from_json(CLAYCO, "ticket")
        ...
        module_data["logs"]["finish"] = DataReference.return_valid_date()
        module_data["logs"]["run-time"] = DataReference.calculate_time_duration(
    ...
    @staticmethod
    def generate_ins():
        DataReference.ynq_user_interaction(
    ...
    def return_valid_file(input_file_dir:str) -> dict|int:
        ...
        if os.path.isdir(input_file_dir):
            file_dict = DataReference._handle_dir(input_file_dir)
        elif os.path.isfile(input_file_dir):
            file_dict = DataReference._handle_file(input_file_dir)
    ...
    def _handle_dir(input_file_dir:str, mode:str="r") -> dict|int:
        if mode in ['u', 'r', 'd']:
            ...
            selection = DataReference._display_options(dir_list)
    ...
    def _handle_file(input_file_dir:str) -> dict|int:
        ...
        if (input_file_extension in DataReference.allowed_extensions):
            return dict(
                path = os.path.dirname(input_file_dir), 
    ...
    @staticmethod
    def read_data_from_library(file_path:str, category:str, extension:"json"):
        if os.path.isfile(file_path):
            print("Error. Provided path is file")
            return None

        basename = f"{category}.{extension}"
        ...
        print("Error. Provided category not in library")
        return None
    ...
    def user_option_to_select_from_library(prompt_message:str, category:str):
        lib_data = DataReference.read_data_from_library(LIBRARY_METADATA, category)

        if lib_data:
            data_arr = [f"{value.name} {value.middle_name}" for _, value in lib_data.items()]
            selection = DataReference._display_options(data_arr)
